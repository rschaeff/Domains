...
our @EXPORT = (
	...
	"&domain_partition_by_hhsearch",
    "&dali_summary",                 "&reference_cache_load",
    "&generate_singleton_run_list",  "&generate_domain_summary_files",
    "&generate_struct_search_jobs",  "&job_list_generate_fasta",
    "&run_list_maintain",            "&job_list_maintain",
    "&jobify_dali",                  "&immediate_dali",
    '$pdb_dir',                      '$pdbml_dir',
    '$status_dir',                   '&buildali_hhblits',
    '&blastp_job_file',              '&chblastp_job_file',
    '&hh_gen',                       '&hh_run',
    '&hh_parse',                     '&register_repair',
    '&reference_library_load',       '&read_self_comps',
    '&struct_search_dali_query_gen', '&generate_query_pdb',
    '&bsumm',                        '&query_glob',
    '&domains',                      '&optimize',
    '&peptide_filter',               '&peptide_collate',
    '&load_partition_vars',          '&find_hh_dali_domains',
    '$DOMAIN_PREFIX',                '$DOMAIN_DATA_DIR',
    '$GAP_TOL',
);
...
my $SINGLE_BOUNDARY_OPTIMIZE =
  '/data/ecod/weekly_updates/weeks/bin/single_boundary_optimize.pl';
my $SINGLE_PEPTIDE_FILTER =
  '/data/ecod/weekly_updates/weeks/bin/single_xml_peptide_prefilter.pl';
...
my $NO_PSIBLAST =
  1;    #Is PSIBLAST providing ANY value over HHSEARCH? It is unclear.
...
sub struct_search_dali_query_gen {
    ...
    #rep find
    my $rep_domains =
      $ecod_xml_doc->findnodes(qq{//domain[\@manual_rep='true']});
    if ($DEBUG) {
    ...
    foreach my $rep_domain_node ( $rep_domains->get_nodelist() ) {
        ...
        if ( $rep_domain_node->findvalue('structure/@structure_obsolete') eq
            'true' )
        {
            next;
        }
        ...
        if ( $ecod_domain_id =~ /e(\w(\w{2})\w)/ ) {
            ...
            if ( !-f $path ) {
                print
"WARNING! NO CLEAN DOMAIN PDB FILE FOR $ecod_domain_id ($path)\n";
            }
            else {
    ...
    my ( $job_dump_dir, $job_list_dir ) =
      get_job_dirs_from_job_xml($job_xml_doc);
    ...
    foreach my $job_node ( $job_nodes->get_nodelist() ) {
        ...
        if ( $reference ne $ref ) {
            print
"WARNING! job reference $reference does not match input refrences $LATEST_REFERENCE\n";
        }
...
#Abandon if DALI summ or domains exists, use only for run completion not overwrite
        ...
        my $dali_summ_fn =
          "$job_dump_dir/$pdb_chain/$pdb_chain.$reference.dali_summ.xml";
        if ( -f $dali_summ_fn ) {
            print
              "WARNING! Dali summary file found for $pdb_chain, skipping...\n";
            next;
        }
        ...
        foreach my $ecod_rep (@ecod_reps) {
            ...
            if ( $DEBUG > 1 ) {
                print
"DALI jobify $query_pdb_chain_fn $DOMAIN_DATA_DIR/$short_uid/$uid/$uid.seqres.pdb\n";
            }
            my $dali_output_fn = "q.$pdb_chain.$ecod_rep.$reference.dali";

            jobify_dali( $dali_tmp_dir, "../../$pdb_chain.pdb",
                "$DOMAIN_DATA_DIR/$short_uid/$uid/$uid.seqres.pdb",
                $dali_output_fn );
        }
...
sub generate_query_pdb {
    ...
    if ($input_seqid_aref) {
        ($coord_aref) =
          pdbml_coord_parse_pull( $pdb, $input_seqid_aref, $asym_id );
    }
    else {
    ...
    my $coord_size = scalar(@$coord_aref)
      ;    #How large is it? Needs to exceed 30 residues for Dali to work
    my $ca_only = 0;
    my %atoms;
  ...
  DP_NODE:
    foreach my $dp_node ( find_domain_parse_nodes($run_list_xml_doc) ) {
            ...
            && !existsMode( $dp_node, $job_generate ) )
        {
            ...
            my $assess_file = get_run_list_job_xml_file( $dp_node, $job_assess )
              ;    #Shouldn't this be $job_assess
            my $generate_file;
   ...
   #my $generate_file_XPath = qq{run_list_job_xml_file[\@mode="$job_generate"]};
            ...
            if ( $assess_file =~ /job/ ) {
                ...
                die
"REGEXP substitute fails on $assess_file for generate_file generation\n";
            }

            my ( $generate_xml_doc, $generate_root_node ) =
              xml_create('job_set_top');

            create_job_dir_nodes_for_job_xml( $generate_xml_doc,
                $generate_root_node, $run_list_dir );

            xml_write( $generate_xml_doc, "$run_list_dir/$generate_file" );

            my $dp_job_xml_file_node =
              $run_list_xml_doc->createElement('run_list_job_xml_file');
            ...
            job_list_assess(
                $run_list_dir, $assess_file,  $generate_file,
                $job_assess,   $job_generate, $DOMAIN_PREFIX
            );

        }
        else {
            print
              "WARNING! No $job_assess node found for $run_id $week_label\n";
        }
    }
}
...
sub job_list_assess {
    ...
    foreach my $aj_node ( find_job_nodes($assess_xml_doc) ) {
        ...
        if ( isPeptide($aj_node) ) {
            if ($DEBUG) {
                print
"DEBUG: $sub: $job_id $query_pdb $query_chain peptide filter apply, skipping...\n";
            }
            next;
        }
        else {
#print "WARNING! $sub: No peptide filter node found for $job_id $query_pdb $query_chain... proceeding anyways\n";
        ...
        if ( isCoiledCoil($aj_node) ) {
            if ($DEBUG) {
                print
"DEBUG: $sub $job_id $query_pdb $query_chain coiled coil filter apply, skipping...\n";
            }
            next;
        }
        else {
#print "WARNING! $sub: No coiled coil filter node for $job_id $query_pdb $query_chain...\n";
        ...
        if ( $mode eq $job_assess ) {
            if (
                job_assess(
                    $run_list_dir,  $query_pdb, $query_chain,
                    $domain_prefix, $reference, $job_assess
                )
              )
            {
                #SUCCESS
    ...
    my $as_job_list_node =
      $generate_xml_doc->findnodes('//job_list')->get_node(1);
...
sub job_assess {
    ...
    if ($DEBUG) {
        print
"DEBUG: $sub: top $run_list_dir, $query_pdb, $query_chain, $domain_prefix, $reference, $job_assess\n";
    }

    my $pdb_chain = $query_pdb . "_" . $query_chain;

    my $fp =
"$run_list_dir/ecod_dump/$pdb_chain/$domain_prefix.$pdb_chain.$reference.xml";
    if ( -f $fp ) {
        ...
        my $chain_domain_coverage =
          $domain_xml_doc->findvalue('//chain_domain_coverage');
        if ( $domain_xml_doc->exists('//optimized_chain_domain_coverage') ) {
            my $unused_residues = $domain_xml_doc->findvalue(
                '//optimized_chain_domain_coverage/@unused_res');
...
sub generate_singleton_run_list {
    ...
    if ( !$run_label ) {
        if ( $job_list_file =~ /(.*)\/(repair.develop\d+)/ ) {
            ...
            die
"ERROR! Could not surmise run_label from job_list_file (assume looks like repair.developN)\n";
        }
    }
    else {
        $dir = ".";
    ...
    my $domain_parse_run_node =
      $run_list_xml_doc->createElement('domain_parse_run');
...
sub generate_domain_summary_files {
    ...
    my $weekly_update_dir =
      $run_list_summary_doc->findvalue('//weekly_update_dir');
    ...
    for ( my $i = 0 ; $i < scalar(@run_data) ; $i++ ) {
        ...
        if ( $process eq 'true' ) {
            ...
            symlink( $run_list_domain_summary_file,
                "$weekly_update_dir/run_list.latest.domain_summary.xml" );
    ...
    foreach my $node ( $run_list_nodes->get_nodelist() ) {
        ...
        if ( $runs[0]{domain_summary_file} ) {
            if ( $node->exists('domain_summary_file') ) {
                foreach my $ds_node (
                    $node->findnodes('domain_summary_file')->get_nodelist() )
                {
                    $node->removeChild($ds_node);
                }
            }
            my $ds_node =
              $run_list_summary_doc->createElement('domain_summary_file');
...
sub run_process_v2 {
    ...
    my ( $domain_summary_doc, $domain_summary_top_node ) =
      xml_create('domain_parse_summary');

    #Copy the run_list_summary head data to the domain summary file
    my $ds_run_list_file_node =
      $domain_summary_doc->createElement('domain_summary_run_list_file');
    $ds_run_list_file_node->appendTextNode($run_list_file);

    my $ds_run_list_label_node =
      $domain_summary_doc->createElement('domain_summary_run_list_label');
    $ds_run_list_label_node->appendTextNode($run_list_label);

    my $ds_run_list_desc_node =
      $domain_summary_doc->createElement('domain_summary_run_list_description');
    $ds_run_list_desc_node->appendTextNode($run_list_desc);

    #Head node for chain list
    my $ds_chain_list_node =
      $domain_summary_doc->createElement('pdb_chain_list');
...
#Go read the run_list summary, the associated hora job list summaries, and the associate domain xmls and build
    ...
    if ( $run_list_xml_doc->exists($run_list_XPath) ) {
        ...
        foreach my $run_node ( $run_list_nodes->get_nodelist() ) {
            ...
            if ( !-d $run_list_dir ) {
                print
"WARNING! $sub: id $id dir $run_list_dir not found, skipping...\n";
                next;
            }
            foreach my $jx_file_node (
                $run_node->findnodes('run_list_job_xml_file')->get_nodelist() )
            {
...
#domain_mode_process_v2($domain_summary_doc, $ref_xml_doc, $run_mode, $reference, $run_node, $run_list_dir, $run_list_label, $domain_prefix);
                domain_mode_process_v2(
                    $domain_summary_doc, $ref_domain_list,
                    $run_mode,           $reference,
                    $run_node,           $run_list_dir,
                    $run_list_label,     $domain_prefix
                );
            }
...
#Only use all of one type
        ...
        print
"WARNING! $run_list_file doesn't have XPath $run_list_XPath, skipping...\n";
        return 0;
    }
    my $domain_summary_file_name =
      "$output_directory/$run_list_label.$domain_prefix.domain_summary.xml";
...
sub domain_merge_v2 {
    ...
    foreach my $pc_node (
        $domain_summary_doc->findnodes($pdb_chain_XPath)->get_nodelist() )
    {
        ...
        foreach my $dp_node ( $pc_node->findnodes('domain_parse') ) {
            ...
            foreach
              my $dom_node ( $dp_node->findnodes('job/domain_list/domain') )
            {
...
#my $dp_node 	= $pc_node->findnodes(qq{domain_parse[\@type="$types[0]"]})->get_node(1);
            ...
            my $dp_cdc_XPath =
              qq{domain_parse[\@type="$types[0]"]/chain_domain_coverage};
            my $dp_cdc_node;

            my $dp_ocdc_XPath =
qq{domain_parse[\@type="$types[0]"]/optimized_chain_domain_coverage};
            my $dp_ocdc_node;
            ...
            if ( $pc_node->exists($dp_cdc_XPath) ) {
                ...
                $dp_cdc_node =
                  $domain_summary_doc->createElement('chain_domain_coverage');
                $dp_cdc_node->appendTextNode('0');
            }

            if ( $pc_node->exists($dp_ocdc_XPath) ) {
                $dp_ocdc_node =
                  $pc_node->findnodes($dp_ocdc_XPath)->get_node(1);
            }
            else {
                $dp_ocdc_node = $domain_summary_doc->createElement(
                    'optimized_chain_domain_coverage');
...
#my $cdc 	= $dp_node->findvalue('chain_domain_coverage');
                ...
                my $dp_node =
                  $pc_node->findnodes(qq{domain_parse[\@type="$type"]})
                  ->get_node(1);
                if ( $dp_node->exists('optimized_chain_domain_coverage') ) {
                    my $ocdc = $dp_node->findvalue(
                        'optimized_chain_domain_coverage/@unused_res');
                    if ( $ocdc && $ocdc <= 25 )
                    {    # ==0 to <= 25 Thursday, May 09 2013
...
#my $dp_node 	= $pc_node->findnodes(qq{domain_parse[\@type="$use_type"]})->get_node(1);
            ...
            my $dp_cdc_XPath =
              qq{domain_parse[\@type="$use_type"]/chain_domain_coverage};
            my $dp_cdc_node;

            my $dp_ocdc_XPath =
qq{domain_parse[\@type="$use_type"]/optimized_chain_domain_coverage};
            my $dp_ocdc_node;
        ...
        $domain_summary_doc->findnodes($pdb_chain_XPath)->get_nodelist() )
    {
        ...
        foreach my $dp_node ( $pc_node->findnodes('domain_parse') ) {
            ...
            foreach
              my $dom_node ( $dp_node->findnodes('job_asm/domain_list/domain') )
            {
                push( @{ $dp_domains{$type} }, $dom_node );
            }
...
sub add_week_list {
    ...
    my $dom_summary_doc_top_node =
      $domain_summary_doc->findnodes('//domain_parse_summary')->get_node(1);

    if ( $domain_summary_doc->exists('//pdb_chain[@week_label]') ) {
        my $week_label_nodes =
          $domain_summary_doc->findnodes('//pdb_chain[@week_label]');
...
sub best_hit_comp {
    ...
    if ( $ver eq 'scop1.69' ) {
        $scop_cla_file =
          '/home/rschaeff/data/scop/v1.69/dir.cla.scop.txt_1.69.txt';
    }
    elsif ( $ver eq 'scop1.75' ) {
        $scop_cla_file =
          '/home/rschaeff/data/scop/v1.75/dir.cla.scop.txt_1.75.txt';
    }
    else {
    ...
    if ( !$dom1_ln ) {
        print
"WARNING! $sub: Domain information for $scop_domid1 not found in $scop_cla_file\n";
    }
    if ( !$dom2_ln ) {
        print
"WARNING! $sub: Domain information for $scop_domid2 not found in $scop_cla_file\n";
    }
...
sub domain_mode_process_v2 {
    ...
    if ( existsMode( $run_node, $mode ) ) {
        ...
        if ( !-f "$run_list_dir/$rl_job_file" ) {
            print
"WARNING! $sub: $mode job xml file $rl_job_file not found, skipping $run_list_dir...\n";
            return 0;
        }
    }
    else {
        print
"WARNING! $sub: no job xml file found for $run_list_dir, $domain_prefix, $mode, skipping...\n";
        return 0;
    }
    ...
    if ( $run_node->exists($rl_week_label_XPath) ) {
        ...
        print
"WARNING! $sub: No week label found for $run_list_dir, $domain_prefix, $mode...\n";
    }
    ...
    if ( $job_xml_doc->exists($job_XPath) ) {
        ...
        if ( $fhx_doc_nodes->size() > 1 ) {
            die
"ERROR! $sub: No support for multiple job docs per file in $rl_job_file, aborting...\n";
        }
        else {
        ...
        if ( $fhx_doc_node->exists($job_list_dir_XPath) ) {
            ...
            print
"ERROR! $sub: Fouled XPath/Broken HorA xml for job list dir on $rl_job_file\n";
            next;
        }
        ...
        if ( $fhx_doc_node->exists($job_dump_dir_XPath) ) {
            ...
            print
"ERROR! $sub: Fouled XPath/Broken HorA xml for job dump dir on $rl_job_file\n";
            next;
        }
...
"DEBUG $sub: Found %i jobs in run list dir $run_list_dir for $run_list_label(mode $mode/ reference $reference) \n",
        ...
        foreach my $job_node ( $job_nodes->get_nodelist() ) {
            ...
            if ( $DEBUG > 2 ) {
                print
                  "DEBUG $sub: $query_pdb $query_chain $job_mode $reference\n";
            }
            my $rep95 = 'false';
            ...
            if ( $job_node->exists('peptide_filter') ) {
                my $peptide_filter_nodes =
                  $job_node->findnodes('peptide_filter');
                my @vers;
                foreach my $pf_node ( $peptide_filter_nodes->get_nodelist() ) {
                    push( @vers, $pf_node->findvalue('@ver') );
                }
                @vers = sort { $b <=> $a } @vers;
                $peptide_filter_nodes =
                  $job_node->findnodes("peptide_filter[\@ver='$vers[0]']");
                my $peptide_filter_node;
                if ( $peptide_filter_nodes->size() > 1 ) {
                    warn
"WARNING! multiple same version peptide filter nodes in $job_inx, $run_list_dir, weird...\n";
            ...
            if ( $job_node->exists('coiled_coil_filter') ) {
                ...
                @vers = sort { $b <=> $a } @vers;
                $cc_nodes =
                  $job_node->findnodes("coiled_coil_filter[\@ver='$vers[0]']");
                my $cc_node;
                if ( $cc_nodes->size() > 1 ) {
                    die
"ERROR! multiple same version cc filter nodes in $job_inx, $run_list_dir, weird...\n";
                }
                else {
            ...
            my $ds_pdb_chain_list_XPath =
              '//domain_parse_summary/pdb_chain_list';
            my (
                $ds_pdb_chain_list_node, $ds_pdb_chain_node,
                $ds_job_node,            $ds_domain_parse_node
            );
            if ( $domain_summary_doc->exists($ds_pdb_chain_list_XPath) ) {
                $ds_pdb_chain_list_node =
                  $domain_summary_doc->findnodes($ds_pdb_chain_list_XPath)
                  ->get_node(1);

                my $ds_pdb_chain_XPath =
                  "pdb_chain[\@pdb=\'$query_pdb\'][\@chain=\'$query_chain\']";

                if ( $ds_pdb_chain_list_node->exists($ds_pdb_chain_XPath) ) {
                    my $ds_pdb_chain_nodes =
                      $ds_pdb_chain_list_node->findnodes($ds_pdb_chain_XPath);
                    if ( $ds_pdb_chain_nodes->size() > 1 ) {
                        die
"ERROR! $sub: More than one pdb_chain node for $query_pdb, $query_chain... $run_list_dir\n";
                    }
                    else {
                    ...
                    $ds_pdb_chain_node =
                      $domain_summary_doc->createElement('pdb_chain');

                    $ds_pdb_chain_node->setAttribute( 'pdb',   $query_pdb );
                    $ds_pdb_chain_node->setAttribute( 'chain', $query_chain );
                    $ds_pdb_chain_node->setAttribute( 'week_label',
                        $rl_week_label );
                    $ds_pdb_chain_node->setAttribute( 'peptide_filter',
                        $peptide_filter );
                    $ds_pdb_chain_node->setAttribute( 'coiled_coil_filter',
                        $coiled_coil_filter );
                    $ds_pdb_chain_node->setAttribute( 'mc_domain_component',
                        $mc_domain_component );
...
"WARNING! $sub: domain parse for $query_pdb,$query_chain,$job_mode,$reference,parser already exists?\n";
                    ...
                    $ds_domain_parse_node =
                      $domain_summary_doc->createElement('domain_parse');
            ...
            if ( $mode eq 'seq_iter' ) {
                $domain_xml_file =
"$job_dump_dir/$pdb_chain/$domain_prefix.$pdb_chain.$reference.xml";
            }
            elsif ( $mode eq 'struct_search' ) {
                $domain_xml_file =
"$job_dump_dir/$pdb_chain/$domain_prefix.$pdb_chain.$reference.struct_search.xml";
            }
          ...
          #print "WARNING! $sub: Domain XML file $domain_xml_file not found!\n";
                $bad_domain_xml_file_count++
                  ;    #Just missing, usually because not rep95 chain
                next;
            }
            ...
            my $domain_doc_XPath =
              "//chain_domains_set_top[\@domain_prefix='$domain_prefix']";
            if ( $domain_xml_doc->exists($domain_doc_XPath) ) {
                my $domain_doc_nodes =
                  $domain_xml_doc->findnodes($domain_doc_XPath);
...
"ERROR! $sub: No support for multiple doc nodes per domain file in $domain_xml_file, aborting...\n";
                ...
                if ( $domain_xml_doc->findvalue('@known_reference_peptide') eq
                    'true' )
                {
                    $ds_pdb_chain_node->setAttribute( 'peptide_filter',
                        'true' );
                ...
                if ( lc($pdb_id) ne lc($query_pdb) ) {
                    print
"WARNING $sub! PDB I/O mismatch for $pdb_id/$query_pdb \n";
                }

                if ( $chain_id ne $query_chain ) {
                    print
"WARNING $sub! CHAIN I/O mismatch for $chain_id/$query_chain";
                }

                if ( $job_mode ne $domain_mode && $domain_mode ne 'blast' ) {
                    print
"WARNING $sub! MODE I/O mismatch for $job_mode/$domain_mode for $job_inx\n";
                }
           ...
           #my $coverage	= $domain_doc_node->findvalue('chain_domain_coverage');
                ...
                my $coverage_node =
                  $domain_doc_node->findnodes('chain_domain_coverage')
                  ->get_node(1);
...
"ERROR! $sub: Chain coverage node already exists for $job_inx, $query_pdb, $query_chain, $run_list_dir?\n";
     ...
     #print "WARNING! $sub: No optimized coverage node for $pdb_id $chain_id\n";
                    ...
                    my $optimized_coverage_node = $domain_doc_node->findnodes(
                        'optimized_chain_domain_coverage')->get_node(1);
                    if (
                        !$ds_domain_parse_node->exists(
                            'optimized_chain_domain_coverage')
                      )
                    {
                        $ds_domain_parse_node->appendChild(
                            $optimized_coverage_node);
                    }
                    else {
                        die
"ERROR! $sub: Optimized chain coverage node already exists for $job_inx, $query_pdb, $query_chain, $run_list_dir?\n";
                ...
                my $domain_list_nodes =
                  $domain_doc_node->findnodes($domain_list_XPath);

                my $ds_domain_list_node;
                if ( !$ds_job_node->exists('domain_list') ) {
                    $ds_domain_list_node =
                      $domain_summary_doc->createElement('domain_list');
...
"ERROR! $sub: Domain list already exists for $job_inx, $query_pdb, $query_chain, $run_list_dir?\n";
                ...
                foreach my $domain_node ( $domain_list_nodes->get_nodelist() ) {
                    ...
                    my $hit_domain_id =
                      $domain_node->findvalue('hit_domain/@ecod_domain_id');
...
"WARNING! $hit_domain_id not found in reference?  $reference $domain_id ($rl_week_label)\n";
                ...
                my $coil_list_nodes =
                  $domain_doc_node->findnodes($coil_list_XPath);

                my $ds_coil_list_node;
                if ( !$ds_job_node->exists('coil_list') ) {
                    $ds_coil_list_node =
                      $domain_summary_doc->createElement('coil_list');
...
"ERROR! $sub: Coil list already exists for $job_inx, $query_pdb, $query_chain, $run_list_dir?\n";
                ...
                print
"ERROR! $sub: Fouled XPath/broken domain XML for top node on $domain_xml_file\n";
            }
        ...
        foreach my $job_asm_node ( $job_asm_nodes->get_nodelist() ) {
            ...
            if ( $DEBUG > 2 ) {
                print
                  "DEBUG $sub: $query_pdb $query_chains $job_mode $reference\n";
            }
            my $rep95 = 'false';
            ...
            if ( $job_asm_node->exists('peptide_filter') ) {
                my $peptide_filter_nodes =
                  $job_asm_node->findnodes('peptide_filter');
                my @vers;
                foreach my $pf_node ( $peptide_filter_nodes->get_nodelist() ) {
                    push( @vers, $pf_node->findvalue('@ver') );
                }
                @vers = sort { $b <=> $a } @vers;
                $peptide_filter_nodes =
                  $job_asm_node->findnodes("peptide_filter[\@ver='$vers[0]']");
                my $peptide_filter_node;
                if ( $peptide_filter_nodes->size() > 1 ) {
                    die
"ERROR! multiple same version peptide filter nodes in $job_inx, $run_list_dir, weird...\n";
                }
                else {
                    $peptide_filter_node = $peptide_filter_nodes->get_node(1);
                }
            ...
            if ( $job_asm_node->exists('coiled_coil_filter') ) {
                ...
                @vers = sort { $b <=> $a } @vers;
                $cc_nodes = $job_asm_node->findnodes(
                    "coiled_coil_filter[\@ver='$vers[0]']");
                my $cc_node;
                if ( $cc_nodes->size() > 1 ) {
                    die
"ERROR! multiple same version cc filter nodes in $job_inx, $run_list_dir, weird...\n";
            ...
            my (
                $ds_pdb_chain_list_node, $ds_pdb_chain_node,
                $ds_job_asm_node,        $ds_domain_parse_node
            );
            if ( $domain_summary_doc->exists($ds_pdb_chain_list_XPath) ) {
                $ds_pdb_chain_list_node =
                  $domain_summary_doc->findnodes($ds_pdb_chain_list_XPath)
                ...
                my $ds_pdb_chain_XPath =
                  "pdb_chain[\@pdb=\'$query_pdb\'][\@chains=\'$query_chains\']";
...
"ERROR! $sub: More than one pdb_chain node for $query_pdb, $query_chains... $run_list_dir\n";
                    ...
                    $ds_pdb_chain_node =
                      $domain_summary_doc->createElement('pdb_chains');
...
"WARNING! $sub: domain parse for $query_pdb,$query_chains,$job_mode,$reference,parser already exists?\n";
                    ...
                    $ds_job_asm_node =
                      $domain_summary_doc->createElement('job_asm');
                    $ds_domain_parse_node =
                      $domain_summary_doc->createElement('domain_parse');
            ...
            if ( $mode eq 'seq_iter' ) {
                $domain_xml_file =
"$job_dump_dir/$pdb_chains/$domain_prefix.$pdb_chains.$reference.xml";
            }
            elsif ( $mode eq 'struct_search' ) {
                $domain_xml_file =
"$job_dump_dir/$pdb_chains/$domain_prefix.$pdb_chains.$reference.struct_search.xml";
            }

            if ( !-f $domain_xml_file ) {
                print
"WARNING! $sub: Domain XML file $domain_xml_file not found!\n";
                $bad_domain_xml_file_count++;
                next;
            }
...
"WARNING $sub! PDB I/O mismatch for $pdb_id/$query_pdb \n";
                ...
                if ( $chain_str ne $query_chains ) {
                    print
"WARNING $sub! CHAINS I/O mismatch for $chain_str/$query_chains";
                }
...
"WARNING $sub! MODE I/O mismatch for $job_mode/$domain_mode for $job_inx\n";
           ...
           #my $coverage	= $domain_doc_node->findvalue('chain_domain_coverage');
                if ( !$domain_doc_node->exists('chain_domain_coverage') ) {
                    die
                      "ERROR! $sub: No coverage node for $pdb_id $chain_str\n";
                }
                my $coverage_node =
                  $domain_doc_node->findnodes('chain_domain_coverage')
                  ->get_node(1);
...
"ERROR! $sub: Chain coverage node already exists for $job_inx, $query_pdb, $query_chains, $run_list_dir?\n";
                ...
                {
                    print
"WARNING! $sub: No optimized coverage node for $pdb_id $chain_str\n";
                }
                else {
                    my $optimized_coverage_node = $domain_doc_node->findnodes(
                        'optimized_chain_domain_coverage')->get_node(1);
...
"ERROR! $sub: Fouled XPath/broken domain XML for top node on $domain_xml_file\n";
        ...
        print
"ERROR! $sub: Fouled XPath/broken job XML for top node on $rl_job_file\n";
        next;
    }
    if ( $bad_domain_xml_file_count > 0 ) {
        print
"WARNING! $rl_job_file has $bad_domain_xml_file_count missing domain files!\n";
    }
    return 0;
}
...
sub register_repair {
    ...
    my ( $ecod_master_ref_inx_fn, $ecod_master_repair_inx_fn, $run_list_file )
      = @_;

    if ( !-f $ecod_master_ref_inx_fn ) {
        die
"ERROR! $sub: ECOD master reference index not found, $ecod_master_ref_inx_fn\n";
    }
    if ( !-f $ecod_master_repair_inx_fn ) {
        die
"ERROR! $sub ECOD master repair run index not found, $ecod_master_repair_inx_fn\n";
    }
    ...
    if ( $ecod_master_repair_xml->exists('//run_list_summary') ) {
        $run_list_summary_node =
          $ecod_master_repair_xml->findnodes('//run_list_summary')->get_node(1);
    }
    else {
...
sub buildali_hhblits {
    ...
    foreach my $node ( $job_list_nodes->get_nodelist() ) {
        ...
        if ( !-f $fa_file ) {
            print
              "WARNING! No FA file ($fa_file) for $query_pdb $query_chain\n";
            next;
        }
...
#my $bsumm_script = '/data/ecod/weekly_updates/weeks/bin/single_domain_summary.pl';
    ...
    foreach my $node ( $job_nodes->get_nodelist() ) {
        ...
        if ( !$use_reps || $reps && $node->findvalue('@rep95') eq 'true' ) {
            $bsumm_script =
              '/data/ecod/weekly_updates/weeks/bin/single_domain_summary.pl';
        }
        else {
            $bsumm_script =
'/data/ecod/weekly_updates/weeks/bin/single_domain_summary.pl --concise';
        }
        ...
        my $bsumm_cmd =
          "$bsumm_script $query_pdb $query_chain $reference $job_dump_dir";
...
sub hh_run {
    ...
    foreach my $node ( $job_nodes->get_nodelist() ) {
        ...
        if ( $query_reference ne $reference ) {
            print
"WARNING! query reference $query_reference does not match input reference $reference\n";
            next;
        }
        ...
        if ( !-f $result_fn || $FORCE_HH_OVERWRITE ) {
            ...
            my $hh_cmd =
"$HHSEARCH_EXE -i $ecod_dir/$hhm_fn -o $result_fn -d $HH_REF{$reference} -cpu 8\n";
            push( @jobs, $hh_cmd );
            job_create( $job_fn, \@jobs );
            my $job_id = qsub($job_fn);
            push( @job_ids, $job_id );
...
sub hh_parse {
    ...
    if ( $job_xml_doc->exists('//job_asm') )
    {    #Refactor this to XML::Grishin::has_job_asm()
        my $nodelist = find_job_asm_nodes($job_xml_doc);
        $job_nodes->append($nodelist);
    }
...
"WARNING! query reference $query_reference does not match input reference $reference\n";
        ...
        if ( !-f $summ_fn || $FORCE_HH_OVERWRITE ) {
            ...
            my $input = 'struct_seqid';
            my $hh_cmd =
"$hh_parse_script $query_pdb $query_chain $result_fn $reference $summ_fn $input";
...
sub optimize {
    ...
    if ( $job_nodes->size() == 0 ) {
        ...
        foreach my $job_node ( find_job_nodes($job_xml_doc) ) {
            ...
            if ( !-f $query_pdb_chain_fn || $FORCE_OVERWRITE )
            {    #generate query pdb
                print "gen: $query_pdb_chain_fn\n";
                if ( !generate_query_pdb( $query_pdb_chain_fn, $pdb, $chain ) )
                {
            ...
            if ( $mode eq 'seq_iter' ) {
                ...
                $domain_dir = "$job_dump_dir/$pdb_chain";
                $domain_fn =
                  "$domain_prefix.$pdb_chain.$reference.struct_search.xml";
            }
            else {
            ...
            if ( !-f "$domain_dir/$domain_fn" ) {
                print
"WARNING! Could not find domain file $domain_dir $domain_fn, skipping...\n";
                next;
            }
            else {
#boundary_optimize($pdb_chain, $domain_dir, $domain_fn, $recurse, $recurse_range);
                ...
                if ($force_overwrite) {
                    print
"$SINGLE_BOUNDARY_OPTIMIZE $pdb_chain $domain_dir $domain_fn --force_overwrite \n";
                    $optim_cmd =
"$SINGLE_BOUNDARY_OPTIMIZE $pdb_chain $domain_dir $domain_fn --force_overwrite\n";
                }
                else {
                    print
"$SINGLE_BOUNDARY_OPTIMIZE $pdb_chain $domain_dir $domain_fn \n";
                    $optim_cmd =
"$SINGLE_BOUNDARY_OPTIMIZE $pdb_chain $domain_dir $domain_fn \n";
...
sub peptide_filter {
    ...
    my @peptide_job_ids;
    foreach my $job_node ( find_job_nodes($job_xml_doc) ) {
        my ( $query_pdb, $query_chain ) = split '_',
          job_node_pdb_chain($job_node);
        my $pc = $query_pdb . "_" . $query_chain;

        my $cmd =
          "$SINGLE_PEPTIDE_FILTER $query_pdb $query_chain $job_dump_dir/$pc/\n";
        my @jobs;
        push( @jobs, $cmd );
...
sub peptide_collate {
    ...
    foreach my $job_node ( find_job_nodes($job_xml_doc) ) {
        ...
        if ( -f $path ) {
            ...
            {
                $job_node->appendChild(
                    $peptide_xml_doc->findnodes('//peptide_filter')->get_node(1)
                );
            }
        }
    }
...
#MOVE executable locations
    my $single_domain_partition =
      '/data/ecod/weekly_updates/weeks/bin/single_domain_partition_v13.pl'
      ;    #Experimental
    if ( !-f $single_domain_partition ) {
        die
"ERROR! Could not find single domain partition script $single_domain_partition\n";
    }
    load_references();
    load_partition_vars();
    ...
    my ( $ref_range_cache, $ref_domain_uid_lookup ) =
      reference_cache_load($global_reference);
    ...
    foreach my $node ( $job_list_nodes->get_nodelist() ) {
        ...
        my $id   = $node->findvalue('@id');
        my $mode = $node->findvalue('mode')
          ;    #Jobs have either seq_iter or struct_search mode
        if ( $mode ne 'seq_iter' ) { next }
...
#	}
        ...
        if ( !-d $node_dir ) {
            print
"WARNING! Node dir ($node_dir) not found for $query_pdb, $query_chain! Skipping...\n";
            next;
        }
...
sub chblastp_job_file {
    ...
    foreach my $node ( $job_list_nodes->get_nodelist() ) {
        ...
        my $blast_fn =
          "$job_dump_dir/$pdb_chain/$pdb_chain.$reference.chainwise_blast.xml";
...
#print `$BLAST_CMD -query $job_dump_dir/$pdb_chain/$pdb_chain.fa  -db $CHAIN_REF{$reference} -outfmt 5 -num_alignments 5000 -evalue 0.002 > $job_list_dump_dir/$pdb_chain/$pdb_chain.$reference.chainwise_blast.xml`;
        ...
        my $job_fn =
          "$job_dump_dir/$pdb_chain/chblastp.$pdb_chain.$reference.job";
...
sub blastp_job_file {
    ...
    foreach my $node ( $job_list_nodes->get_nodelist() ) {
        ...
        if (   $force_overwrite
            ...
            || -s "$job_dump_dir/$pdb_chain/$pdb_chain.$reference.blast.xml" ==
            0 )
        {
            my $blast_cmd =
...
sub job_list_maintain {
    my (
        $job_list_xml_fn, $new_week,        $reference,
        $reps_only,       $force_overwrite, $no_peptide
    ) = @_;
    ...
    foreach my $job_node ( find_job_nodes($job_xml_doc) ) {
        ...
        if ( -f $fasta_fn && $force_overwrite < 2 ) {
            ...
            my $ungapped_struct_range =
              ungap_range( $struct_seqid_range, $GAP_TOL );
            ...
            my $fasta_string =
              pdbml_fasta_fetch( $query_pdb, $asym_id, $query_chain,
                $ungap_aref );
        ...
        foreach my $job_asm_node ( find_job_asm_nodes($job_xml_doc) ) {
            ...
            if ( !-d "$job_dump_dir/$pdb_chain_str" ) {
                if ( !mkdir("$job_dump_dir/$pdb_chain_str") ) {
                    die
                      "ERROR! Could not create $job_dump_dir/$pdb_chain_str\n";
                }
                else {
            ...
            if ( -f $fasta_fn && $force_overwrite < 2 ) {
                ...
                foreach my $query_chain (@$query_chain_aref) {
                    ...
                    {
                        print
"WARNING! No struct_seq for $query_pdb, $query_chain\n";
                    ...
                    if ( $ungapped_struct_range eq 0 ) {
                        print
                          "WARNING! empty range for $query_pdb, $query_chain\n";
                        next;
                    }
                    ...
                    if ( !$tmp_fasta_string ) {
                        print
"WARNING! No fasta string for $query_pdb, $query_chain\n";
                        next;
                    }
                    else {
    ...
    my $cluster_script =
      '/data/ecod/weekly_updates/weeks/bin/run_list_cluster.pl';
    if ( !-f $cluster_script ) {
    ...
    #Build profiles
    my $buildali_script =
      '/data/ecod/weekly_updates/weeks/bin/xml_hhblits_profile_build.pl';
...
#my $self_comp_script = '/home/rschaeff/work/domain_partition/ecod_domain_parser/bin/ecod_self_comparison.pl';
    my $self_comp_script =
      '/data/ecod/weekly_updates/weeks/bin/ecod_self_comparison.pl';
    ...
    my $chblast_job_ids =
      chblastp_job_file( $job_list_xml_fn, $reps_only, $force_overwrite );
    ...
    my $blast_job_ids =
      blastp_job_file( $job_list_xml_fn, $reps_only, $force_overwrite );
    ...
    my $domains_job_ids =
      domains( $job_list_xml_fn, $reference, 1, $FORCE_OVERWRITE );
...
sub dali_summary {
    ...
    my $DALI_SUMM_EXE =
      '/data/ecod/weekly_updates/weeks/bin/single_dali_summary.pl';
    ...
    foreach my $job_node ( $job_nodes->get_nodelist() ) {
        ...
        my ( $seqid_aref, $struct_seqid_aref, $pdbnum_href, $asym_id ) =
          pdbml_seq_parse( $pdb, $chain );

        if ($recurse) {
            $seqid_aref = isect( range_expand($recurse_range), $seqid_aref );
            $struct_seqid_aref =
              isect( range_expand($recurse_range), $struct_seqid_aref );
        }
        ...
        if (0) {
            ...
            foreach my $ecod_rep (@ecod_reps) {
                ...
                my $dali_file =
                  "$dali_tmp_dir/q.$pdb_chain.$ecod_rep.$reference.dali";
...
sub build_rep_stats {
    ...
    foreach my $rep_node ( $node_list->get_nodelist() ) {
        ...
        my (
            $ecod_seqid_aref,  $ecod_struct_seqid_aref,
            $ecod_pdbnum_href, $ecod_asym_id
        ...
        my ($ecod_seq) = pdbml_seq_fetch( $ecod_pdb, $ecod_asym_id, $ecod_chain,
            $ecod_seqid_aref );
        if ($ecod_seqid_aref) {
...
sub run_list_maintain {
    ...
    my ( $run_list_xml_fn, $run_label, $run_dir, $reference, $start_date_ctime )
      = @_;
    my $run_list_xml_doc;
    ...
    if ( -f $run_list_xml_fn ) {
        ...
        my ( $run_list_xml_doc, $run_list_doc_node ) =
          xml_create('domain_parse_run_list');
    ...
    open( my $fh, "<", $obsolete_file )
      or die
      "ERROR! Could not open obsolete manifest file $obsolete_file: $!\n";
    while ( my $ln = <$fh> ) {
    ...
    my $run_list_head_node =
      find_first_node( $run_list_xml_doc, "domain_parse_run_list" );
    ...
    foreach my $new_week (@new_week_dirs) {
        ...
        if ( !open( $fh, "<", "$status_dir/$new_week/added.pdb" ) ) {
            die
"ERROR! Could not open $status_dir/$new_week/added.pdb for reading:$!\n";
        ...
        if ( $warning > 10 ) {
            print
"WARNING! week $new_week not complete (missing_pdb = $warning), skipping\n";
            next;
        }
        ...
        my $run_list_dir_node =
          $run_list_xml_doc->createElement('run_list_dir');
        ...
        if ( -f $job_list_xml_fn ) {
            print
"WARNING! $job_list_xml_fn exists, set \$FORCE_OVERWRITE to overwrite\n";
            $job_xml_doc = xml_open($job_list_xml_fn);
            ( $job_dump_dir, $job_list_dir ) =
              get_job_dirs_from_job_xml($job_xml_doc);
        }
        elsif ( !-f $job_list_xml_fn || $FORCE_OVERWRITE ) {

            my $domain_parse_job_xml_node =
              $run_list_xml_doc->createElement('run_list_job_xml_file');
            $domain_parse_job_xml_node->appendTextNode(
                "$new_week.$run_label.job.xml");
            ...
            my $run_list_document_node =
              $job_xml_doc->createElement("job_set_top");
            ...
            for ( my $i = 0 ; $i < scalar(@new_pdbs) ; $i++ ) {
                ...
                for ( my $j = 0 ; $j < scalar(@$chain_aref) ; $j++ ) {
                    ...
                    my $query_pdb_node =
                      $job_xml_doc->createElement('query_pdb');
                    $query_pdb_node->appendTextNode($pdb);

                    my $query_chain_node =
                      $job_xml_doc->createElement('query_chain');
                    $query_chain_node->appendTextNode($chain);

                    my $reference_node =
                      $job_xml_doc->createElement('reference');
...
#my $fasta_string = pdbml_fasta_fetch($query_pdb, $asym_id, $query_chain, $struct_seqid_aref);
        ...
        open( $fh, ">", "$run_label.$new_week.cluster.job" )
          or die
          "Could not open $run_label.$new_week.cluster.job for writing:%!\n";
        print $fh "#!/bin/bash\n";
        print $fh "#\$ -cwd\n";
        print $fh "#\$ -j y \n";
        print $fh "#\$ -S /bin/bash\n";
...
#my $self_comp_script = '/home/rschaeff/work/domain_partition/ecod_domain_parser/bin/ecod_self_comparison.pl';
        ...
        open( OUT, ">$run_label.$new_week.self_comp.job" )
          or die
          "Could not open $run_label.$new_week.self_comp.job for writing:$!\n";
        print OUT "#!/bin/bash\n";
        print OUT "#\$ -cwd\n";
        print OUT "#\$ -j y \n";
        print OUT "#\$ -S /bin/bash\n";
    ...
    if ($DALI_SEARCH) {
        foreach
          my $run_node ( $run_list_xml_doc->findnodes('//domain_parse_run') )
        {
            if ( existsMode( $run_node, 'struct_search' ) ) {
                my $run_list_dir    = $run_node->findvalue('run_list_dir');
                my $job_list_xml_fn = $run_node->findvalue(
                    'run_list_job_xml_file[@mode="struct_search"]');

                my $job_list_xml_path = "$run_list_dir/$job_list_xml_fn";
                struct_search_dali_query_gen($job_list_xml_path);
                my $dali_jobs_aref = query_glob($job_list_xml_path);
                throttled_qsub( $dali_jobs_aref, 20000 );
                my $dali_summ_job_ids =
                  dali_summary( $job_list_xml_path, $LATEST_REFERENCE );
...
sub domain_partition_asm {
    ...
    my (
        $query_pdb,         $query_chains, $dir,
        $input_mode,        $reference,    $domain_xml_fn,
        $ref_range_cache,   $concise,      $global_opt,
        $ref_chain_domains, $ref_domain_uid_lookup
    ) = @_;
    ...
    if ( -f $domain_xml_fn && !$$global_opt{force_overwrite} ) {
        print
"WARNING! $sub: skipping $query_pdb, $query_chains_str, set FORCE_OVERWRITE bit to override...\n";
        return;
    }
    my (
        $query_seqid_aref,  $query_struct_seqid_aref,
        $query_pdbnum_aref, $query_asym_id,
        $query_chain_aref,  $query_struct_chain_aref
    ...
    for ( my $i = 0 ; $i < scalar(@$query_seqid_aref) ; $i++ ) {
        $super_seqid_sort{ $$query_chain_aref[$i] }{ $$query_seqid_aref[$i] } =
          $i;
        $super_seqid_pdbnum_lookup{$i} =
          "$$query_chain_aref[$i]$$query_seqid_aref[$i]";
    ...
    for ( my $i = 0 ; $i < scalar(@$query_struct_seqid_aref) ; $i++ ) {
        $super_struct_seqid_sort{ $$query_struct_chain_aref[$i] }
          { $$query_struct_seqid_aref[$i] } = $i;

        $super_struct_seqid_pdbnum_lookup{$i} =
          "$$query_struct_chain_aref[$i]$$query_struct_seqid_aref[$i]";
    ...
    my $query_range_str =
      multi_chain_rangify( $query_seqid_aref, $query_chain_aref );
    my $query_struct_range_str =
      multi_chain_rangify( $query_struct_seqid_aref, $query_struct_chain_aref );
...
#Changed struct_seqid to be ungapped (i.e. may contain small unstructured regions);
    ...
    my $query_ungapped_struct_range_str =
      multi_chain_ungap_range( $query_struct_range_str, $$global_opt{gap_tol} );
    ( $query_struct_seqid_aref, $query_struct_chain_aref ) =
      multi_chain_range_expand($query_ungapped_struct_range_str);

    if ( !$query_struct_range_str ) {
        print
"WARNING! $sub: $query_pdb $query_chains_str.. No structured range $query_struct_range_str\n";
    }
    ...
    if ($DEBUG) {
        print
"DEBUG $sub: $query_pdb $query_chains_str $dir $query_range_str $query_struct_range_str $domain_xml_fn\n";
    }
    ...
    if ( !-f $blast_summ_fn ) {
        if ( -f "$dir/$pdb_chains.$reference.blast_summ.concise.xml" ) {
            $blast_summ_fn =
              "$dir/$pdb_chains.$reference.blast_summ.concise.xml";
        }
        else {
            print
"WARNING! $sub: BLAST summary for $query_pdb, $query_chains_str, $dir ($blast_summ_fn) not found\n";
            next;
        }
    }
    ...
    my $domain_doc_node =
      $domain_xml_doc->createElement('chain_domains_set_top');
    ...
    my $blast_summ_pdb =
      $blast_summ_xml_doc->findvalue('//blast_summ_doc/blast_summ/@pdb');
    ...
    {
        print
"ERROR! Blast file pdb mismatch $query_pdb/$blast_summ_pdb $query_chains_str/$blast_summ_chains\n";
        die;
    }

    #Self_comparison read:
    my $self_comp_run_XPath = q{//blast_summ_doc/blast_summ/self_comp_run};
    my $self_comp_run_nodes =
      $blast_summ_xml_doc->findnodes($self_comp_run_XPath);
    ...
    if ( $self_comp_run_nodes_size < 1 ) {
        ...
        my $self_comp_hit_nodes =
          $self_comp_run_nodes->get_node(1)->findnodes('hits/hit');
        my $ci = 0;
        foreach my $hit_node ( $self_comp_hit_nodes->get_nodelist() ) {
            ...
            if ( $aligner eq 'dali' ) {
                ...
                print
"WARNING! Unknown self_comparison aligner $aligner, skipping...\n";
                next;
            }
    ...
    #Chainwise BLAST
    my $chblastp_run_XPath =
      q{//blast_summ_doc/blast_summ/chain_blast_run[@program='blastp']};
    my $chblastp_run_nodes =
      $blast_summ_xml_doc->findnodes($chblastp_run_XPath);
    my $chblastp_run_nodes_size = $chblastp_run_nodes->size();
    if ( $chblastp_run_nodes_size != 1 ) {
        print
"ERROR! $sub: Odd number of chblastp run nodes found ($chblastp_run_nodes_size), aborting...\n";
    }
    my $chblastp_hit_nodes =
      $chblastp_run_nodes->get_node(1)->findnodes('hits/hit');
    foreach my $hit_node ( $chblastp_hit_nodes->get_nodelist() ) {
        ...
        if ( $input_mode eq 'seqid' ) {
            ...
            ( $hit_query_struct_seqid_aref, $hit_query_struct_chain_aref ) =
              multi_chain_struct_region( range_expand($query_reg),
                $query_seqid_aref, $query_chain_aref );
        }
        ...
        if ($DEBUG) {
            print
"DEBUG $sub: chblastp $hit_num $hit_pdb $hit_chain $query_struct_seqid_range\n";
        }
        ...
        my $used_coverage =
          multi_chain_region_coverage( $query_struct_seqid_aref,
            $query_struct_chain_aref, \@used_seq, \@used_chain );
...
"DEBUG $sub: uc $used_coverage uuc $unused_coverage qnc $query_new_coverage qoc $query_used_coverage\n";
            ...
            && $query_used_coverage < $$global_opt{old_coverage_threshold} )
        {
            ...
            my (
                $hit_seqid_aref,  $hit_struct_seqid_aref,
                $hit_pdbnum_aref, $hit_asym_id,
                $hit_chain_aref,  $hit_struct_chain_aref
            );
            (
                $hit_seqid_aref,  $hit_struct_seqid_aref,
                $hit_pdbnum_aref, $hit_asym_id
            ...
            print
"DEFINE chblastp: $sub: ($hit_pdb $hit_chain $hit_reg) ->  ($query_struct_seqid_range)\n";
            ...
            for ( my $i = 0 ; $i < scalar(@hit_seqs) ; $i++ ) {
                ...
                if ($DEBUG) {
                    print
                      "DEBUG $sub: chblastp struct_range_expand 1 $hit_seg\n";
                }

                my ( $hit_seg_seqid_aref, $hit_seg_chain_aref );
                $hit_seg_seqid_aref = struct_region( range_expand($hit_seg),
                    $hit_struct_seqid_aref );
...
"DEBUG $sub: chblastp struct_range_expand 2 $query_seg %s\n",
                ...
                if ( $input_mode eq 'seqid' ) {
                    ...
                    $query_seg_seqid_aref = isect( range_expand($query_seg),
                        $query_struct_seqid_aref );
                }
                elsif ( $input_mode eq 'struct_seqid' ) {
                    $query_seg_seqid_aref =
                      struct_region( range_expand($query_seg),
                        $query_struct_seqid_aref );
                    $query_seg_chain_aref =
                      struct_region( range_expand($query_seg),
...
#}
            ...
            foreach
              my $hit_domain_uid ( @{ $$ref_chain_domains{$hit_pdb}{$hit_chain} } )
            {

                my $ref_domain_id =
                  $$ref_range_cache{$hit_domain_uid}{ecod_domain_id};

                my $hit_domain_id_node =
                  $domain_xml_doc->createElement('hit_domain');
                $hit_domain_id_node->setAttribute( 'ecod_domain_id',
                    $ref_domain_id );
                $hit_domain_id_node->setAttribute( 'reference', $reference );
                $hit_domain_id_node->setAttribute( 'uid', $hit_domain_uid );

                my $ref_seqid_range = multi_chain_ungap_range(
                    $$ref_range_cache{$hit_domain_uid}{seqid_range}, $GAP_TOL );

                if ($DEBUG) {
                    print
"DEBUG: $sub: chblastp_ref_domain $ref_domain_id $ref_seqid_range\n";
                }
...
#"MESS $ref_seqid $hit_map{$ref_seqid} $ref_seg_chain \n";
                                push( @query_struct_seqid_range,
                                    $hit_map{$ref_seqid} );

                              #push (@query_struct_chain_range, $ref_seg_chain);
                                push( @query_struct_chain_range,
                                    $hit_chain_map{$ref_seqid} );
                            }
                            else {
                ...
                my $struct_seqid_range_node =
                  $domain_xml_doc->createElement('struct_seqid_range');

           #$struct_seqid_range_node->appendTextNode($query_struct_seqid_range);
                if ( scalar(@query_struct_seqid_range) == 0 ) {
                    print
"WARNING! $sub: No struct seqid for $query_pdb $query_chains chblastp, serious problem...\n";
                    next;
                }
                my $query_struct_seqid_range =
                  multi_chain_rangify( \@query_struct_seqid_range,
                    \@query_struct_chain_range );
                $struct_seqid_range_node->appendTextNode(
                    $query_struct_seqid_range);
                $domain_node->appendChild($struct_seqid_range_node);

                my $ungapped_seqid_range =
                  multi_chain_ungap_range( $query_struct_seqid_range,
                    $$global_opt{gap_tol} );
                my ( $ungapped_seqid_range_aref, $ungapped_chain_range_aref ) =
                  multi_chain_range_expand($ungapped_seqid_range);
                my $ungapped_seqid_range_node =
                  $domain_xml_doc->createElement('ungapped_seqid_range');
                $ungapped_seqid_range_node->setAttribute( 'gap_tolerance',
                    $$global_opt{gap_tol} );
                $ungapped_seqid_range_node->appendTextNode(
                    $ungapped_seqid_range);
                $domain_node->appendChild($ungapped_seqid_range_node);

                #REPLACE WITH HIT DOMAIN RANGE (THAT OVERLAPS ALLGNED REGION)
                my $hit_struct_seqid_range_node =
                  $domain_xml_doc->createElement('hit_struct_seqid_range');

                #$hit_struct_seqid_range_node->appendTextNode($hit_hit_reg);
                my $hit_struct_seqid_range;
                $hit_struct_seqid_range =
                  scopify_range( rangify(@hit_struct_seqid_range), $hit_chain );
                ...
                my $pdb_range =
                  multi_chain_pdb_rangify( \@query_struct_seqid_range,
                    $query_pdbnum_aref, \@query_struct_chain_range );
                my $pdb_range_node =
                  $domain_xml_doc->createElement('pdb_range');
                $pdb_range_node->appendTextNode($pdb_range);
                $domain_node->appendChild($pdb_range_node);

                my $ungapped_pdb_range =
                  multi_chain_pdb_rangify( $ungapped_seqid_range_aref,
                    $query_pdbnum_aref, $ungapped_chain_range_aref );
                my $ungapped_pdb_range_node =
                  $domain_xml_doc->createElement('ungapped_pdb_range');
                $ungapped_pdb_range_node->setAttribute( 'gap_tolerance',
                    $$global_opt{gap_tol} );
                $ungapped_pdb_range_node->appendTextNode($ungapped_pdb_range);
                $domain_node->appendChild($ungapped_pdb_range_node);

                my ( $range_str, $chain_str ) =
                  scop_range_split($ungapped_pdb_range);
                ...
                my $domain_id =
                  "e" . lc($query_pdb) . "$domain_id_chain_str$domain_count";
                $domain_node->setAttribute( 'domain_id', $domain_id );

                my $blast_score_node =
                  $domain_xml_doc->createElement('blast_eval');
    ...
    #BLAST
    my $blastp_run_XPath =
      q{//blast_summ_doc/blast_summ/blast_run[@program='blastp']};
    ...
    if ( $blastp_run_nodes_size != 1 ) {
        print
"ERROR! $sub: Odd number of blastp run nodes found ($blastp_run_nodes_size), aborting...\n";

    }
    my $blastp_hit_nodes =
      $blastp_run_nodes->get_node(1)->findnodes('hits/hit');
    foreach my $hit_node ( $blastp_hit_nodes->get_nodelist() ) {
        ...
        $hit_domain_id =~ /(e|d)(\w{4})(\w+)/
          ; #Define overall hit domain regexp elsewhere, time to obsolete SCOP domains in hits.
...
#my $hit_query_struct_seqid_range = rangify(@$hit_query_struct_seqid_aref);
        ...
        my ( $hit_query_struct_seqid_aref, $hit_query_struct_chain_aref ) =
          range_decompose(
            $hit_query_reg,    $query_struct_seqid_aref,
            $query_seqid_aref, $query_struct_chain_aref,
            $query_chain_aref, $input_mode
          );
        ...
        if ($DEBUG) {
            print
"DEBUG $sub: blastp $hit_num $hit_domain_id $hit_query_struct_seqid_range\n";
        }
...
"DEBUG $sub: uc $used_coverage uuc $unused_coverage qnc $query_new_coverage qoc $query_used_coverage\n";
        ...
        {
            print
"DEFINE blastp: $sub: $hit_domain_id $hit_query_struct_seqid_range\n";
            ...
            if ( $hit_chain eq '.' ) {
                my $domain_range =
                  $$ref_range_cache{$hit_domain_uid}{seqid_range};
                my ( $chain_str, $chain_range ) =
                  scop_range_split($domain_range);
            ...
            if ( $hit_chain eq '.' ) {
                ...
                $hit_reg_struct_seqid_aref =
                  struct_region( range_expand($hit_hit_reg),
                    $hit_struct_seqid_aref );
            }
       ...
       #$domain_node->setAttribute('domain_id', $domain_id);
            ...
            my $struct_seqid_range_node =
              $domain_xml_doc->createElement('struct_seqid_range');
            $struct_seqid_range_node->appendTextNode(
                $hit_query_struct_seqid_range);
            $domain_node->appendChild($struct_seqid_range_node);

            my $ungapped_seqid_range =
              multi_chain_ungap_range( $hit_query_struct_seqid_range,
                $$global_opt{gap_tol} );
            ...
            if ( $hit_chain eq '.' ) {
                $hit_struct_seqid_range_node->appendTextNode(
                    multi_chain_rangify(
                        $hit_reg_struct_seqid_aref,
                        $hit_reg_struct_chain_aref
                    )
                );
            }
            else {
                $hit_struct_seqid_range_node->appendTextNode(
                    rangify(@$hit_reg_struct_seqid_aref) );
            }
 ...
 #my $pdb_range = pdb_rangify($hit_query_struct_seqid_aref, $query_pdbnum_aref);
            my $pdb_range =
              multi_chain_pdb_rangify( $hit_query_struct_seqid_aref,
                $query_pdbnum_aref, $hit_query_struct_chain_aref );
...
#my $ungapped_pdb_range = pdb_rangify($ungapped_seqid_range_aref, $query_pdbnum_aref);
            my $ungapped_pdb_range =
              multi_chain_pdb_rangify( $ungapped_seqid_range_aref,
                $query_pdbnum_aref, $ungapped_seqid_chain_aref );
            my $ungapped_pdb_range_node =
              $domain_xml_doc->createElement('ungapped_pdb_range');
            $ungapped_pdb_range_node->setAttribute( 'gap_tolerance',
                $$global_opt{gap_tol} );
            ...
            $hit_domain_id_node->setAttribute( 'ecod_domain_id',
                $hit_domain_id );
            $hit_domain_id_node->setAttribute( 'reference', $reference );
            $hit_domain_id_node->setAttribute( 'uid',
                $$ref_domain_uid_lookup{$hit_domain_id} );
            $domain_node->appendChild($hit_domain_id_node);
      ...
      #range_exclude(range_expand($hit_query_struct_seqid_range), \@unused_seq);
        ...
        #range_include(range_expand($hit_query_struct_seqid_range), \@used_seq);
            ...
            for ( my $i = 0 ; $i < scalar(@self_comps) ; $i++ ) {
                ...
                my $sc_query_coverage_1 = region_coverage( $sc_query_reg_aref,
                    $ungapped_seqid_range_aref );
                my $sc_query_coverage_2 =
                  region_coverage( $ungapped_seqid_range_aref,
                    $sc_query_reg_aref, );
                my $sc_hit_coverage_1 =
                  region_coverage( $sc_hit_reg_aref, \@unused_seq );
...
"DEFINE blastp self_comp $sub: $self_comp_aligner qr $self_comp_query_reg hr $self_comp_hit_reg pu $pre_used pun $pre_unused\n";
                    ...
                    my $sc_domain_node =
                      $domain_xml_doc->createElement('domain');

                    my $domain_id =
                      'e' . lc($query_pdb) . "$query_chains$domain_count";
                    $sc_domain_node->setAttribute( 'domain_id', $domain_id );

                    my $method_node = $domain_xml_doc->createElement('method');
                    my $method      = 'blastp_self_comp';
                    $method_node->appendTextNode($method);
                    $method_node->setAttribute( 'self_comp_aligner',
                        $self_comp_aligner );
                    ...
                    my $ungapped_seqid_range =
                      ungap_range( $self_comp_hit_reg, $$global_opt{gap_tol} );
                    my $ungapped_seqid_range_aref =
                      range_expand($ungapped_seqid_range);
                    my $ungapped_seqid_range_node =
                      $domain_xml_doc->createElement('ungapped_seqid_range');
                    $ungapped_seqid_range_node->setAttribute( 'gap_tolerance',
                        $$global_opt{gap_tol} );
                    ...
                    my $pdb_range =
                      pdb_rangify( $sc_hit_reg_aref, $query_pdbnum_aref );
                    my $pdb_range_node =
                      $domain_xml_doc->createElement('pdb_range');
                    $pdb_range_node->appendTextNode($pdb_range);
...
"WARNING! self comp aligner $self_comp_aligner unknown\n";
    ...
    my $final_coverage = multi_chain_region_coverage( $query_struct_seqid_aref,
        $query_struct_chain_aref, \@used_seq, \@used_chain );
    my $chain_domain_coverage_node =
      $domain_xml_doc->createElement('chain_domain_coverage');
...
sub domain_assignment_by_hhsearch { 
    ...
    if ( -f $domain_xml_fn && !$$global_opt{force_overwrite} ) {
        print
"WARNING! $sub: skipping $query_pdb, $query_chain, set FORCE_OVERWRITE bit to override...\n";
        return;
    }
    ...
    my (
        $struct_href,          $exptl_href,          $entity_href,
        $target_entity_id,     $struct_ref_href,     $citation_href,
        $citation_author_href, $entity_src_nat_href, $entity_src_gen_href,
        $entity_src_syn_href
...
#Changed query sequence to be ungapped (i.e. may contain small unstructured regions, extremely important that GAP_TOL is the same for query generation as domain partition).
    my $query_ungapped_struct_range_str =
      ungap_range( $query_struct_range_str, $$global_opt{gap_tol} );
    my @gapped_query_struct_seqid = @$query_struct_seqid_aref
      ;    #Need to retain this for dali_fillin (query pdbs are not ungapped)
    $query_struct_seqid_aref = range_expand($query_ungapped_struct_range_str);

    if ( !$query_struct_range_str ) {
        print
"WARNING! $sub: $query_pdb $query_chain,  No structured range found =>  $query_struct_range_str\n";
    }
    ...
    if ($DEBUG) {
        print
"DEBUG $sub: $query_pdb $query_chain $dir $query_range_str $query_struct_range_str $domain_xml_fn\n";
    }
    my $blast_summ_fn;
...
#}
    ...
    if ( $src_method eq 'syn' ) {
        print
          "WARNING! $query_pdb, $query_chain is a known synthetic product\n";
    ...
    my $blast_summ_pdb =
      $blast_summ_xml_doc->findvalue('//blast_summ_doc/blast_summ/@pdb');
    my $blast_summ_chain =
      $blast_summ_xml_doc->findvalue('//blast_summ_doc/blast_summ/@chain');

    find_hhsearch_domains(
        $blast_summ_xml_doc, $domain_xml_doc,
        $ref_chain_domains,  $ref_domain_uid_lookup,
        $ref_range_cache,    $query_struct_seqid_aref,
        $query_seqid_aref,   $query_pdbnum_aref,
        $self_comp_aref,     \@used_seq,
        \@unused_seq,        $input_mode,
    ...
    my $final_coverage =
      region_coverage( $query_struct_seqid_aref, \@used_seq );
...
#Changed query sequence to be ungapped (i.e. may contain small unstructured regions, extremely important that GAP_TOL is the same for query generation as domain partition).
    ...
    if ( !$query_struct_range_str ) {
        print
"WARNING! $sub: $query_pdb $query_chain NO structured range $query_struct_range_str\n";
    }
...
#}
    ...
    if ( !-f $blast_summ_fn ) {
        if ( -f "$dir/$pdb_chain.$reference.blast_summ.concise.xml" ) {
            $blast_summ_fn =
              "$dir/$pdb_chain.$reference.blast_summ.concise.xml";
        }
        else {
            print
"WARNING! $sub: BLAST summary for $query_pdb, $query_chain, $dir ($blast_summ_fn) not found\n";
            next;
        }
    }
    ...
    if ( $query_pdb ne $blast_summ_pdb || $query_chain ne $blast_summ_chain ) {
        ...
        print
"ERORR! hora file/blast file pdb mismatch $query_pdb/$blast_summ_pdb $query_chain/$blast_summ_chain\n";
        die;
    }
    ...
    find_chainwise_blast_domains(
        $blast_summ_xml_doc,    $domain_xml_doc,
        $ref_chain_domains,     $ref_range_cache,
        $ref_domain_uid_lookup, $query_struct_seqid_aref,
        $query_seqid_aref,      $query_pdbnum_aref,
        \@used_seq,             \@unused_seq,
        $input_mode,            $global_opt,
        $reference,             \@coils,
    );
...
#my $self_comp_hit_nodes = $self_comp_run_nodes->get_node(1)->findnodes('hits/hit');
        my $self_comp_hit_nodes = $blast_summ_xml_doc->findnodes(
'//blast_summ_doc/blast_summ/self_comp_run/repeat_set_list/repeat_hits/hit'
        );
        my $ci = 0;
        foreach my $hit_node ( $self_comp_hit_nodes->get_nodelist() ) {
            ...
            #my $aligner 	= $hit_node->findvalue('@aligner');
            my $aligner =
              $hit_node->parentNode->parentNode->parentNode->findvalue(
                '@programs');
...
"WARNING! Unknown self_comparison aligner $aligner, skipping...\n";
        ...
        $self_comp_hit_nodes = $blast_summ_xml_doc->findnodes(
            '//blast_summ_doc/blast_summ/self_comp_run/hits/hit');
        foreach my $hit_node ( $self_comp_hit_nodes->get_nodelist() ) {

            #my $aligner 	= $hit_node->findvalue('@aligner');
            my $aligner =
              $hit_node->parentNode->parentNode->findvalue('@programs');
...
sub define_coils {
    ...
    my $domain_doc_node =
      $domain_xml_doc->findnodes('//chain_domains_set_top')->get_node(1);
    ...
    foreach my $coil (@$coil_aref) {
        ...
        my $query_coverage =
          region_coverage( $coil_seqid_aref, $unused_seq_aref );
        my $query_used_coverage =
          region_coverage( $coil_seqid_aref, $used_seq_aref );
            ...
            && $query_used_coverage < $$global_opt{old_coverage_threshold} )
        {
            ...
            my $ungapped_seqid_range =
              ungap_range( $coil_seqid_str, $$global_opt{gap_tol} );
            ...
            my $ungapped_pdb_range =
              pdb_rangify( $ungapped_seqid_range_aref, $query_pdbnum_aref );
            my $ungapped_pdb_range_node =
              $domain_xml_doc->createElement('ungapped_pdb_range');
            $ungapped_pdb_range_node->setAttribute( 'gap_tolerance',
                $$global_opt{gap_tol} );
...
sub find_hhsearch_domains {
    ...
    my (
        ...
        $ref_range_cache,    $query_struct_seqid_aref,
        $query_seqid_aref,   $query_pdbnum_aref,
        $self_comp_aref,     $used_seq_aref,
        $unused_seq_aref,    $input_mode,
        $global_opt,         $reference,
        $coil_aref
    ) = @_;
    ...
    my $domain_list_node =
      $domain_xml_doc->findnodes('//domain_list')->get_node(1);

    my $query_pdb   = $domain_xml_doc->findvalue('//@pdb_id');
    my $query_chain = $domain_xml_doc->findvalue('//@chain_id');

    my $hhsearch_run_XPath =
      q{//blast_summ_doc/blast_summ/hh_run[@program='hhsearch']};
    my $hhsearch_run_nodes =
      $blast_summ_xml_doc->findnodes($hhsearch_run_XPath);
    my $hhsearch_run_nodes_size = $hhsearch_run_nodes->size();
    if ( $hhsearch_run_nodes_size > 1 ) {
        print
"ERROR! $sub: Odd number of hhsearch run nodies found {$hhsearch_run_nodes_size), aborting...\n";
    }

    if ( $hhsearch_run_nodes_size == 1 ) {
        my $hhsearch_hit_nodes =
          $hhsearch_run_nodes->get_node(1)->findnodes('hits/hit');
        foreach my $hit_node ( $hhsearch_hit_nodes->get_nodelist() ) {
            ...
            if ( $hit_node->findvalue('@hit_cover') <
                $$global_opt{hit_coverage_threshold} )
            {    #what
                print
"Skipping $hit_num $hit_domain_id $$global_opt{hit_coverage_threshold}\n";
                next;
            }
...
#9/20/2012 why would this be translated from pos-inx to seqid? It's already in seqid?
            my $hit_query_struct_seqid_aref;
            if ( $input_mode eq 'seqid' ) {
                $hit_query_struct_seqid_aref =
                  struct_region( range_expand($hit_query_reg),
                    $query_seqid_aref );
            }
            elsif ( $input_mode eq 'struct_seqid' ) {
                $hit_query_struct_seqid_aref =
...
"WARNING! No structured query range? $hit_domain_id $hit_query_reg $query_struct_seqid $$query_struct_seqid_aref[0] $$query_struct_seqid_aref[1] %i\n",
            ...
            {
                print
"WARNING! No range for hit $hit_num $hit_domain_id $query_pdb $query_chain\n";
                next;
            }
...
"DEBUG $sub: hhsearch $hit_num $hit_domain_id socore:$hh_score posx:$hit_query_reg seqid:$hit_query_struct_seqid\n";
            ...
            my $unused_coverage =
              region_coverage( $query_seqid_aref, $unused_seq_aref );
            ...
            my $query_coverage =
              region_coverage( $hit_query_struct_seqid_aref, $unused_seq_aref );
            my $query_used_coverage =
              residue_coverage( $hit_query_struct_seqid_aref, $used_seq_aref );
            if ($DEBUG) {
                my $range1 = rangify(@$hit_query_struct_seqid_aref);
                my $range2 = rangify(@$unused_seq_aref);
                my $query_residue_coverage =
                  residue_coverage( $hit_query_struct_seqid_aref,
                    $unused_seq_aref );
...
"DEBUG $sub: r1 $range1 r2 $range2 qrc $query_residue_coverage urc $unused_rescount qrescount $query_rescount\n";
                ...
                $$global_opt{gap_tol} )
            {
                ...
                print
"DEFINE hhsearch: $sub: $hit_domain_id $hit_query_struct_seqid\n";

                my $domain_node = $domain_xml_doc->createElement('domain');
                my $domain_id =
                  "e" . lc($query_pdb) . "$query_chain$domain_count";
                $domain_node->setAttribute( 'domain_id', $domain_id );
                ...
                my $ungapped_seqid_range =
                  ungap_range( $hit_query_struct_seqid, $$global_opt{gap_tol} );
                my $ungapped_seqid_range_aref =
                  range_expand($ungapped_seqid_range);
                my $ungapped_seqid_range_node =
                  $domain_xml_doc->createElement('ungapped_seqid_range');
                ...
                my $pdb_range = pdb_rangify( $hit_query_struct_seqid_aref,
                    $query_pdbnum_aref );
                my $pdb_range_node =
                  $domain_xml_doc->createElement('pdb_range');
                $pdb_range_node->appendTextNode($pdb_range);
                ...
                for ( my $i = 0 ; $i < scalar(@$self_comp_aref) ; $i++ ) {
                    ...
                    my $sc_query_coverage_1 =
                      region_coverage( $sc_query_reg_aref,
                        $ungapped_seqid_range_aref );
                    my $sc_query_coverage_2 =
                      region_coverage( $ungapped_seqid_range_aref,
                        $sc_query_reg_aref, );
                    my $sc_hit_coverage_1 =
                      region_coverage( $sc_hit_reg_aref, $unused_seq_aref );
...
"DEBUG hhsearch SC $i $self_comp_aligner qr $self_comp_query_reg hr $self_comp_hit_reg sqc1 $sc_query_coverage_1 sqc2 $sc_query_coverage_2 shc1 $sc_hit_coverage_1\n";
                    ...
                    {
                        print
"DEFINE hhsearch self_comp $sub: $self_comp_aligner\n";
                        ...
                        my $domain_id =
                          'e' . lc($query_pdb) . "$query_chain$domain_count";
                        $sc_domain_node->setAttribute( 'domain_id',
                            $domain_id );

                        my $method_node =
                          $domain_xml_doc->createElement('method');
                        ...
                        my $ungapped_seqid_range =
                          ungap_range( $self_comp_hit_reg,
                            $$global_opt{gap_tol} );
                        my $ungapped_seqid_range_aref =
                          range_expand($ungapped_seqid_range);
                        my $ungapped_seqid_range_node =
                          $domain_xml_doc->createElement(
                            'ungapped_seqid_range');
                        $ungapped_seqid_range_node->setAttribute(
                            'gap_tolerance', $$global_opt{gap_tol} );
                        $ungapped_seqid_range_node->appendTextNode(
                            $ungapped_seqid_range);
                        $sc_domain_node->appendChild(
                            $ungapped_seqid_range_node);
                        ...
                        $hit_domain_id_node->setAttribute( 'ecod_domain_id',
                            $hit_domain_id );
                        $hit_domain_id_node->setAttribute( 'reference',
                            $reference );
                        $hit_domain_id_node->setAttribute( 'uid',
                            $$ref_domain_uid_lookup{$hit_domain_id} );
                        ...
                        if ( $self_comp_aligner eq 'dali' ) {
                            my $dali_score_node =
                              $domain_xml_doc->createElement('dali_zscore');
                            $dali_score_node->appendTextNode(
                                $$self_comp_aref[$i]{zscore} );
                            $sc_domain_node->appendChild($dali_score_node);
                        }
                        elsif ( $self_comp_aligner eq 'hhrepid' ) {
                            my $hh_prob_node =
                              $domain_xml_doc->createElement('hhrepid_prob');
                            $hh_prob_node->appendTextNode(
                                $$self_comp_aref[$i]{prob} );
                            $sc_domain_node->appendChild($hh_prob_node);
                        }
                        else {
                            print
"WARNING! self comp aligner $self_comp_aligner unknown\n";
                        ...
                        $domain_list_node->appendChild($sc_domain_node);
                        range_include( $ungapped_seqid_range_aref,
                            $used_seq_aref );
                        range_exclude( $ungapped_seqid_range_aref,
                            $unused_seq_aref );
...
sub find_hh_dali_domains {
    ...
    my (
        $blast_summ_xml_doc,      $dali_summ_xml_doc,
        $domain_xml_doc,          $ref_chain_domains,
        $ref_range_cache,         $ref_domain_uid_lookup,
        $query_struct_seqid_aref, $query_seqid_aref,
        $query_pdbnum_aref,       $self_comp_aref,
        $used_seq_aref,           $unused_seq_aref,
        $input_mode,              $global_opt,
        $reference,               $coil_aref
    ) = @_;
    ...
    foreach my $dali_hit_node ( find_dali_hit_nodes($dali_summ_xml_doc) ) {
        ...
        my ( $hit_reg, $query_reg, $coverage ) =
          get_dali_regions($dali_hit_node);
    ...
    foreach my $hh_hit_node ( find_hhsearch_hit_nodes($blast_summ_xml_doc) ) {
        ...
        if ( $input_mode eq 'seqid' ) {
            $hit_query_struct_seqid_aref =
              struct_region( range_expand($query_reg), $query_seqid_aref );
        }
        elsif ( $input_mode eq 'struct_seqid' ) {
            $hit_query_struct_seqid_aref =
              struct_region( range_expand($query_reg),
                $query_struct_seqid_aref );
        }
            ...
            foreach my $hh_align ( @{ $hh_hits{$key} } ) {
                ...
                if (
                    bidirectional_coverage(
                        ...
                        range_expand( $hh_align->{query_range} ),
                        0.5
                    )
                  )
                {
                    my $mixed_range_aref =
                      union( $dali_range_aref, $hh_range_aref );    #Not isect?
                    ...
                    my $query_coverage =
                      region_coverage( $mixed_range_aref, $unused_seq_aref );
                    my $query_used_coverage =
                      residue_coverage( $mixed_range_aref, $used_seq_aref );
...
#print "qc:$query_coverage quc: $query_used_coverage nct:$$global_opt{new_coverage_threshold}\n";
                        ...
                        && scalar(@$mixed_range_aref) > $$global_opt{gap_tol} )
                    {
                        ...
                        $domain_node->appendTextChild( 'struct_seqid_range',
                            $mixed_range );

                        $domain_node->appendTextChild( 'hit_dali_range',
                            $dali_align->{hit_range} );
                        $domain_node->appendTextChild( 'hit_hh_range',
                            $hh_align->{hit_range} );

                        my $ungapped_seqid_range =
                          ungap_range( $mixed_range, $$global_opt{gap_tol} );
                        my $ungapped_seqid_range_aref =
                          range_expand($ungapped_seqid_range);
                        my $usr_node = $domain_xml_doc->createElement(
                            'ungapped_seqid_range');
                        $usr_node->appendTextNode($ungapped_seqid_range);
                        $usr_node->setAttribute( 'gap_tolerance',
                            $$global_opt{gap_tol} );
                        $domain_node->appendChild($usr_node);

                        $domain_node->appendTextChild(
                            'pdb_range',
                            pdb_rangify(
                                $mixed_range_aref, $query_pdbnum_aref
                            )
                        );
                        ...
                        $ugp_node->setAttribute( 'gap_tolerance',
                            $$global_opt{gap_tol} );
                        $domain_node->appendChild($ugp_node);

                        my $hdi_node =
                          $domain_xml_doc->createElement('hit_domain');
                        $hdi_node->setAttribute( 'ecod_domain_id',
                            $hit_ecod_domain_id );
                        $hdi_node->setAttribute( 'reference', $reference );
                        $hdi_node->setAttribute( 'uid',
                            $$ref_domain_uid_lookup{$hit_ecod_domain_id} );
                        $domain_node->appendChild($hdi_node);

                        my $hh_score_node =
                          $domain_xml_doc->createElement('hh_score');
                        $hh_score_node->setAttribute( 'hh_prob',
                            $hh_align->{hh_prob} );
                        $domain_node->appendChild($hh_score_node);

                        my $dali_score_node =
                          $domain_xml_doc->createElement('dali_score');
                        $dali_score_node->setAttribute( 'z-score',
                            $dali_align->{zscore} );
...
sub find_psiblast_domains {
    ...
    my $psiblast_run_XPath =
      q{//blast_summ_doc/blast_summ/blast_run[@program='psiblast']};
    my $psiblast_run_nodes =
      $blast_summ_xml_doc->findnodes($psiblast_run_XPath);
    my $psiblast_run_nodes_size = $psiblast_run_nodes->size();
    if ( $psiblast_run_nodes_size > 1 ) {
        print
"ERROR! $sub: Odd number of psiblast run nodes found ($psiblast_run_nodes_size), aborting...\n";
    }

    if ( $psiblast_run_nodes_size == 1 && !$$global_opt{no_psiblast} ) {
        my $psiblast_hit_nodes =
          $psiblast_run_nodes->get_node(1)->findnodes('hits/hit');
        foreach my $hit_node ( $psiblast_hit_nodes->get_nodelist() ) {
            ...
            if ($DEBUG) {
                print
"DEBUG $sub: psiblast $hit_num $hit_domain_id $hit_query_struct_seqid_range\n";
            }
            ...
            my $query_coverage =
              region_coverage( range_expand($hit_query_struct_seqid_range),
                $unused_seq_aref );
            ...
            if ( $query_coverage > $$global_opt{new_coverage_threshold} ) {
                print
"DEFINE psiblast: $sub: $hit_domain_id $hit_query_struct_seqid_range\n";
                ...
                my $ungapped_seqid_range =
                  ungap_range( $hit_query_struct_seqid_range,
                    $$global_opt{gap_tol} );
                my $ungapped_seqid_range_aref =
                  range_expand($ungapped_seqid_range);
                my $ungapped_seqid_range_node =
                ...
                range_exclude( range_expand($hit_query_struct_seqid_range),
                    $unused_seq_aref );
                range_include( range_expand($hit_query_struct_seqid_range),
                    $used_seq_aref );
...
"DEBUG psiblast SC $i $self_comp_aligner qr $self_comp_query_reg hr $self_comp_hit_reg sqc1 $sc_query_coverage_1 sqc2 $sc_query_coverage_2 shc1 $sc_hit_coverage_1\n";
                    ...
                    {
                        print
"DEFINE psiblast self_comp $sub: $self_comp_aligner\n";
...
"DEBUG $sub: blastp $hit_num $hit_domain_id $hit_query_struct_seqid_range\n";
        ...
        my $unused_coverage =
          region_coverage( $query_struct_seqid_aref, $unused_seq_aref );
        my $used_coverage =
          region_coverage( $query_struct_seqid_aref, $used_seq_aref );
...
#my $query_coverage = region_coverage(range_expand($hit_query_seqid_range), \@unused_seq);
        my $query_new_coverage =
          region_coverage( $hit_query_struct_seqid_aref, $unused_seq_aref );
        my $query_used_coverage =
          region_coverage( $hit_query_struct_seqid_aref, $used_seq_aref );

        if ($DEBUG) {
            print
"DEBUG $sub: uc $unused_coverage qnc $query_new_coverage qoc $query_used_coverage\n";
        }
...
"DEFINE blastp: $sub: $hit_domain_id $hit_query_struct_seqid_range\n";
            ...
            my $pdb_range =
              pdb_rangify( $hit_query_struct_seqid_aref, $query_pdbnum_aref );
            my $pdb_range_node = $domain_xml_doc->createElement('pdb_range');
            $pdb_range_node->appendTextNode($pdb_range);
            $domain_node->appendChild($pdb_range_node);
...
"ERROR! $sub: Odd number of chblastp run nodes found ($chblastp_run_nodes_size), aborting...\n";
    ...
    foreach my $hit_node ( $chblastp_hit_nodes->get_nodelist() ) {
        ...
        if ( $hit_chain eq '.' ) {
            die
"FATAL ERROR! chain blast not correctly configured to handle multi-chain domain hits\n";
        }
...
"DEBUG $sub: uc $unused_coverage qnc $query_new_coverage qoc $query_used_coverage\n";
    ...
    #ungap_range_aref(@$hit_struct_seqid_aref, $GAP_TOL); #This wasn't working!?
            ...
            print
"DEFINE chblastp: $sub: $hit_pdb $hit_chain $query_struct_seqid_range $test\n";
            ...
            for ( my $i = 0 ; $i < scalar(@hit_segs) ; $i++ ) {
                ...
                my $hit_seg_seqid_aref = struct_region( range_expand($hit_seg),
                    $hit_struct_seqid_aref );
                my @hit_seq = split( "", $hit_seqs[$i] );
...
#my $query_seg_seqid_aref	= struct_region(range_expand($query_seg), $hit_query_struct_seqid_aref);
          ...
          HIT_DOMAIN:
            foreach my $hit_domain_uid (
                @{ $$ref_chain_domains{$hit_pdb}{$hit_chain} } )
            {
                ...
                if ($DEBUG) {
                    print
"DEBUG: $sub: chblastp_domain $ref_domain_id $ref_seqid_range\n";
                }
                my $domain_node = $domain_xml_doc->createElement('domain');
                $domain_node->appendChild($hit_domain_id_node);
                my $domain_id =
...
#Prevent very short chblastp domains (does not solve fragment problem, just too short domain problem)
                ...
                print "DEBUG: scalar qssr: "
                  . scalar(@query_struct_seqid_range) . "\n";
           ...
           #$struct_seqid_range_node->appendTextNode($query_struct_seqid_range);
                if ( scalar(@query_struct_seqid_range) == 0 ) {
                    print
"WARNING! $sub: No struct seqid for $query_pdb $query_chain chblastp, serious problem...\n";
                    next;
                }
                my $query_struct_seqid_range =
                  rangify(@query_struct_seqid_range);
                $struct_seqid_range_node->appendTextNode(
                    $query_struct_seqid_range);
                $domain_node->appendChild($struct_seqid_range_node);

                my $ungapped_seqid_range =
                  ungap_range( rangify(@query_struct_seqid_range),
                    $$global_opt{gap_tol} );
                my $ungapped_seqid_range_aref =
                  range_expand($ungapped_seqid_range);
                my $ungapped_seqid_range_node =
                ...
                my $pdb_range =
                  pdb_rangify( \@query_struct_seqid_range, $query_pdbnum_aref );
                ...
                #REPLACE WITH DOMAIN BASED RANGE
                range_exclude( range_expand($query_struct_seqid_range),
                    $unused_seq_aref );
                range_include( range_expand($query_struct_seqid_range),
                    $used_seq_aref );
...
sub generate_dali_fillin {
  ...
  #Do domains exist to  hit chains with multiple domains (included some unused).
    ...
    foreach my $domain ( find_domain_nodes($domain_xml_doc) ) {
        ...
        print
"D: $domain_id $query_pdb $query_chain => $hit_domain_id/$hit_uid $hit_pdb $hit_chain\n";

        if ( scalar( @{ $$ref_chain_domains{$hit_pdb}{$hit_chain} } ) > 1 ) {
            foreach
              my $hit_uid ( @{ $$ref_chain_domains{$hit_pdb}{$hit_chain} } )
            {
...
#push (@job_ids, $job_id);
    ...
    my $dali_fillin_summ_fn =
      "$pdb_chain_dump_dir/$query_pdb_chain.$reference.fillin.dali_summ.xml";
    if ( 0 && -f $dali_fillin_summ_fn ) {
        print
"WARNING! Dali fillin summary file found ($dali_fillin_summ_fn), skipping...\n";
        return;
    }
    ...
    my $query_seq_href =
      pdbml_seq_fetch( $query_pdb, $query_asym_id, $query_chain,
        $query_seqid_aref );
...
sub find_dali_fillin_domains {
    ...
    my (
        $domain_xml_doc,    $dali_summ_xml_doc,
        $ref_chain_domains, $ref_domain_uid_lookup,
        $ref_range_cache,   $query_struct_seqid_aref,
        $query_pdbnum_aref, $query_asym_id,
        $used_seq_aref,     $unused_seq_aref,
    ...
    for ( my $i = 0 ; $i < scalar(@hits) ; $i++ ) {
        ...
        my $query_domain_seqid_aref =
          struct_region( range_expand($query_reg), $query_struct_seqid_aref );
        my $query_domain_seqid_range =
          ungap_range( rangify(@$query_domain_seqid_aref),
            $$global_opt{gap_tol} );
        my $query_domain_pdb_range =
          ungap_range(
            pdb_rangify( $query_domain_seqid_aref, $query_pdbnum_aref ),
            $$global_opt{gap_tol} );

        my $query_new_coverage =
          region_coverage( $query_domain_seqid_aref, $unused_seq_aref );
        my $query_used_coverage =
          region_coverage( $query_domain_seqid_aref, $used_seq_aref );

        printf "$sub: %.2f %.2f %.2f\n", $query_new_coverage,
          $query_used_coverage, $hit_coverage;
          ...
          )
        {
            ...
            my $fragment_defined =
              $hit_coverage <= $$global_opt{hit_coverage_threshold} ? 1 : 0;

            my $hit_struct_seqid_range;
            if ($mc_hit) {
                my ( $hit_seqid_aref, $hit_struct_seqid_aref, $hit_pdbnum_href,
                    $hit_asym_id, $hit_chain_aref )
                  = pdbml_mc_seq_parse( $hit_pdb, $hit_chain );
                if ( !$hit_seqid_aref ) {
                    print
                      "WARNING! mc hit parse failure, $hit_pdb, $hit_chain\n";
                    next;
                }
                my ( $hit_reg_struct_seqid_aref, $hit_reg_struct_chain_aref ) =
                  multi_chain_struct_region( range_expand($hit_reg),
                    $hit_struct_seqid_aref, $hit_chain_aref );
                $hit_struct_seqid_range =
                  multi_chain_rangify( $hit_reg_struct_seqid_aref,
                    $hit_reg_struct_chain_aref );
            }
            else {
                my (
                ...
                my $hit_reg_struct_seqid_aref =
                  struct_region( range_expand($hit_reg),
                    $hit_struct_seqid_aref );
                my $hit_reg_struct_seqid_range =
                  rangify(@$hit_reg_struct_seqid_aref);
                $hit_struct_seqid_range =
                  scopify_range( $hit_chain, $hit_reg_struct_seqid_range );
            }
            ...
            my $struct_seqid_range_node =
              $domain_xml_doc->createElement('struct_seqid_range');
            $struct_seqid_range_node->appendTextNode(
                rangify(@$query_domain_seqid_aref) );
            $domain_node->appendChild($struct_seqid_range_node);
            ...
            my $ungapped_pdb_range_node =
              $domain_xml_doc->createElement('ungapped_pdb_range');
            $ungapped_pdb_range_node->setAttribute( 'gap_toleraance',
                $GAP_TOL );
            $ungapped_pdb_range_node->appendTextNode($ungapped_pdb_range);
            $domain_node->appendChild($ungapped_pdb_range_node);

            my $hit_domain_node = $domain_xml_doc->createElement('hit_domain');
            $hit_domain_node->setAttribute( 'ecod_domain_id',
                $hit_ecod_domain_id );
